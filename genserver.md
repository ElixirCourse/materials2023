...
* Обработката на съобщението отнема по-дълго от времето, което клиента очаква.
* Все пак съобщението е обработено и отговорът е изпратен.
* Никога това съобщение няма да бъде прочетено от клиента, защото `ref` няма да се съпостави с никой бъдещ `make_ref()`.
...
<img class="center_img" src="assets/save_queue.png" width="80%" />
...
* Задача:
  ...
  * Записването изисква време и ресурси - отваря файл, записва, затваря файл;
  * Ако се записва в база данни - добавя се времето за network заявката.
* Решение:
  * Буфериране на логовете и записването им групово (batch).
  * Кой и как буферира?
...
* Ако `trap_exit: true`, то `terminate/2` ще бъде извикана и при неочаквано спиране на свързан процес.
...
* `init/1` - извиква се при стартиране на процеса,
