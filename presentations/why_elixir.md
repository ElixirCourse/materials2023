...
* Ако сте записали курса за да вземете няколко лесни кредита - няма да много е лесно.
* Ако смятате, че ООП е единственият начин, този курс не е за вас.
* Ако не можете да отделите достатъчно време за домашни и проекти - също ще е трудно.
...
* Провежда се в понеделник и сряда от 19:00 до 21:00.
* Оценката се образува от 5-6 домашни и проект.
* **Няма** тестове, писмен или теоретичен изпит.
* Широк набор от възможности за бонус точки:
...
* **Динамично**- и **силно-типизиран** **функционален** език.
...
* **Динамично-** и **силно-типизиран** **функционален** език, създаден през 1986 г.
* Целта му е да реши проблемите на телеком индустрията:
  1. **Конкурентност**;
  2. **Скалируемост**;
  3. **Дистрибутираност**;
  4. **Толерантност към грешки**;
  5. **Обновяване на кода без спиране на програмта**;
  6. **Бърз отговор**, дори когато системата е под голямо натоварване;
...
* Операторът "=" е опеаратор за съпоставяне (match operator), а не за присвояване (Elixir).
* Позволява писането на по-декларативен код. При правилна употреба води до до по-ясен код.
...
* Можем да разгледаме Elixir като език/технология от 4 слоя:
  ...
  * **Толерантен към грешки** - Процесите се наблюдават взаимо с линкове и монитори. Грешките се прихващат и се обработват. Използват се предефинирани поведения;
...
|> then(fn {microseconds, _} ->
  IO.puts("Completed #{n * k} actions in #{microseconds / 100}ms")
...
* **Apple** - Environment and supply chain inovation (ESCI)
